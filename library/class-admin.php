...
class wssmgk_auto_stock_manager {
	...
	static function wssmgk_main_product_options() {
		...
		?>
		<!-- /* Date and time input */ -->
		<fieldset class="form-field wssmgk_select_start_time" <?php if($wssmgk_schedule == '') { echo 'style="display:none"'; } ?>>
			<legend class="default"><img src="<?php echo WSSMGK_URL .'/library/assets/images/crown.png';?>" alt="crown icon">Start DateTime</legend>
			<legend class="custom_date_and_time" <?php echo $wssmgk_schedule_display; ?>><img src="<?php echo WSSMGK_URL .'/library/assets/images/crown.png';?>" alt="crown icon">Select DateTime</legend>
			<span class="screen-reader-text wssmgkp_start_yy">Year</span>
			<input type="text" id="wssmgkp_date" class="wssmgkp_date" name="wssmgkp_date" value="" placeholder="YYYY-MM-DD" maxlength="10" autocomplete="off" disabled>
			<span>@</span>
			<input type="text" id="wssmgkp_hh" class="wssmgkp_hh" name="wssmgkp_hh" placeholder="HH" value="" size="2" maxlength="2" autocomplete="off" disabled><span>:</span>
	...
	static function wssmgk_save_main_product_options($id){
		...
		if(isset($_POST['_manage_stock'])) $manage_stock=sanitize_text_field($_POST['_manage_stock']); 
		if(isset($_POST['wssmgk_schedule_mode'])) $wssmgk_schedule_mode=sanitize_text_field($_POST['wssmgk_schedule_mode']);
		$wssmgk_old_schedule=get_post_meta($id,'wssmgk_schedule',true);
		if(isset($_POST['wssmgk_schedule'])) $wssmgk_schedule=sanitize_text_field($_POST['wssmgk_schedule']);
	...
	static function wssmgk_variation_product_options($loop, $variation_data, $variation) {
		...
		<fieldset class="form-field wssmgk_select_start_time" <?php if($wssmgk_schedule == '') { echo 'style="display:none"'; } ?>>
			...
			<legend class="custom_date_and_time" <?php echo $wssmgk_schedule_display; ?>><img src="<?php echo WSSMGK_URL .'/library/assets/images/crown.png';?>" alt="crown icon">Select DateTime</legend>
			<span class="screen-reader-text wssmgkp_start_yy">Year</span>
			<input type="text" id="wssmgkp_date[<?php esc_attr_e($variation->ID); ?>]" class="wssmgkp_date" name="wssmgkp_date" value="" placeholder="YYYY-MM-DD" maxlength="10" autocomplete="off" disabled>
			<span>@</span>
			<input type="text" id="wssmgkp_hh[<?php esc_attr_e($variation->ID); ?>]" class="wssmgkp_hh" name="wssmgkp_hh" placeholder="HH" value="" size="2" maxlength="2" autocomplete="off" disabled><span>:</span>
	...
	{	
		...
		if(isset($_POST['wssmgk_schedule_mode'][$post_id])) $wssmgk_schedule_mode=sanitize_text_field($_POST['wssmgk_schedule_mode'][$post_id]);
		if(isset($_POST['wssmgk_schedule'][$post_id])) $wssmgk_schedule=sanitize_text_field($_POST['wssmgk_schedule'][$post_id]);
		if(isset($_POST['wssmgk_stock'][$post_id])) $wssmgk_stock=(isset($_POST['wssmgk_stock'][$post_id])) ? sanitize_text_field($_POST['wssmgk_stock'][$post_id]) : "";
